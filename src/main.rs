    ...
    let matches = App::new("adh-pal")
        ...
        .subcommand(
            App::new("current")
                ...
                .arg(arg!(-c --clear "Clear the current activity.")),
        )
        .subcommand(
            App::new("list")
    ...
    match matches.subcommand() {
        Some(("start", arguments)) => {
            let activity = arguments
                .value_of("ACTIVITY")
                .expect("failed to extract activity");
            cmd::start(activity)
        }
        ...
        Some(("list", arguments)) => match arguments.value_of("NUM") {
            Some(num) => cmd::list_last(num.parse().unwrap()),
            None => cmd::list_all(),
        },
